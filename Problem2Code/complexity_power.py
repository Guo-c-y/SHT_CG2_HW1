...
def _fit_power_law(x: np.ndarray, y: np.ndarray) -> Tuple[float, float, float]:
    ...
    return float(p), float(math.exp(b)), R2  # y ≈ c * n^p
...
def _fit_linear(x: np.ndarray, y: np.ndarray) -> Tuple[float, float, float]:
    ...
    ss_tot = float(np.sum((y - y.mean())**2))
    R2 = float(1 - ss_res/ss_tot) if ss_tot > 0 else float("nan")
    return float(a), float(b), R2  # y ≈ a*x + b

def _fit_nlogn(x: np.ndarray, y: np.ndarray) -> Tuple[float, float, float]:
    """在线性域拟合 y ≈ a * (n log n) + b。"""
    m = x > 1  # 避免 log(1)=0 的退化；你的 n 最小为10，足够
    if np.count_nonzero(m) < 2: return float("nan"), float("nan"), float("nan")
    z = x[m] * np.log(x[m])
    return _fit_linear(z, y[m])  # 返回 a, b, R2
...
def analyze_power_cells_complexity(
    ...
    ns: List[int] = []; t_med: List[float] = []
    ops_total: List[float] = []; hp_counts: List[float] = []; approx_mb: List[float] = []
    ...
    for sites in sites_list:
        ...
        approx_mb.append(hp * 24.0 / (1024.0**2))
    ...
    # 时间：两种模型
    p, c, r2_time_pow = _fit_power_law(ns_arr, t_arr)                   # y ≈ c n^p
    a_nlogn, b_nlogn, r2_time_nlogn = _fit_nlogn(ns_arr, t_arr)         # y ≈ a n log n + b
    ...
    # 选择更优时间模型
    better_time_model = "n^p" if (r2_time_pow >= r2_time_nlogn) else "nlogn"
    ...
    out: Dict[str, Any] = {
        ...
        "fits": {
            "time_powerlaw": {"form": "c*n^p", "p": p, "c": c, "R2": r2_time_pow},
            "time_nlogn":    {"form": "a*n*log n + b", "a": a_nlogn, "b": b_nlogn, "R2": r2_time_nlogn},
            "ops_linear_in_n2": {"a": a_ops, "b": b_ops, "R2": r2_ops},
            "space_linear_in_n2": {"a": a_sp, "b": b_sp, "R2": r2_sp},
            "time_better": better_time_model,
        },
    }
    ...
    if show:
        ...
        if np.isfinite(p) and np.isfinite(c):
            ax0.plot(n_line, c * n_line**p, color="black",
                     label=f"fit n^p: {c:.1g}·n^{p:.2f} (R²={r2_time_pow:.2f})")
        if np.isfinite(a_nlogn) and np.isfinite(b_nlogn):
            ax0.plot(n_line, a_nlogn * (n_line * np.log(n_line)) + b_nlogn, color="gray",
                     label=f"fit nlogn: {a_nlogn:.2e}·nlnn+{b_nlogn:.2e} (R²={r2_time_nlogn:.2f})")
        ax0.set_xticks(xticks)
        # 右移图例，避免与曲线重叠
        ax0.legend(loc="lower left", bbox_to_anchor=(0.22, 0.02))
